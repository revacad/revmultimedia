-- Reuse returning student row via applications.returning_student_id before application_id lookup.
...
BEGIN
  ...
  IF v_application.returning_student_id IS NOT NULL THEN
    SELECT s.id, s.student_id
    INTO v_student_pk, v_student_id
    FROM students s
    WHERE s.id = v_application.returning_student_id
    LIMIT 1;
  END IF;
  ...
  IF v_student_pk IS NULL THEN
    ...
    INTO v_student_pk, v_student_id
    FROM students s
    WHERE s.auth_user_id = v_application.auth_user_id
    ORDER BY s.created_at ASC NULLS LAST, s.id ASC
    LIMIT 1;
  END IF;
